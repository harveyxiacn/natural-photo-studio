...
- Debug 从零配置和编译通过，最终安全修复后完整 CTest 为 `107/107`，并额外连续
  重复完整套件三次，均为 `107/107`；
- Release 从零编译及最终增量安全构建通过，完整 CTest 为 `107/107`；
- Catch2 单元层为 `106` 个用例、`2770` 条断言；
...
- Release 安装树包含 CLI、OpenSSL/SQLite 运行时和许可声明；从安装树实际完成
  ...
  头与完整长度校验；
- npm 公共仓库门禁通过：`99` 个 UTF-8 文本文件、`37` 份 Markdown、`51` 个内部
  链接、`99` 个公开安全扫描文件和 `23/23` 项策略测试；
